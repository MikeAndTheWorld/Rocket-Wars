...
Shader &Shader::Use()
{
	glUseProgram(this->ID); //use shaders
	return *this;
}
...
void Shader::loadFromFile(const GLchar *vShaderFile, const GLchar *fShaderFile) {
	...
	{
		// Open files
		std::ifstream vertexShaderFile(vShaderFile); //vertex
		std::ifstream fragmentShaderFile(fShaderFile); // fragment
...
void Shader::checkCompileErrors(GLuint object, std::string type) //check for errors
{
	GLint success;
	GLchar infoLog[1024];
	if (type != "Program")
