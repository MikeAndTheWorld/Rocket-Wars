...
{
	//Model Matrix needs to be passed to the gpu here.

	GLuint modelLocation = glGetUniformLocation(shader.ID, "mModel");

	Object * obj = getParent();
	glm::mat4 model = obj->getComponent<Transform>()->getModelMatrix();
	glUniformMatrix4fv(modelLocation, 1, GL_FALSE, glm::value_ptr(model));

	glActiveTexture(0);
	//texture.Bind();
	obj->getComponent<Texture>()->Bind();
...
Model::Model(std::string filePath, Object * parent)
{
